...
int LegorduloLista::getSelected()
{
    return _selectedInt;
}

//vector<string> LegorduloLista::getItems()
...
void LegorduloLista::add_item(string item)
{
    _items.push_back(item);
}


void LegorduloLista::remove_item(string item)
{
    if(_items.size() > 0)
    {
        for(size_t i=0; i < _items.size(); i++)
    ...
    {
        cout << "elfogyott az item" << endl;
        actualItem = "elfogyott";
    }
}
...
void LegorduloLista::handle(event ev)
{
    ...
    if (ev.type == ev_mouse && itemFelett(ev.pos_x,ev.pos_y))
    {
        ...
        if (itemFelett(ev.pos_x,ev.pos_y) >= 1 && ev.button == btn_left && ev.type == ev_mouse)
        {
            ...
            _selectedInt = itemFelett(ev.pos_x, ev.pos_y) -1 + eltolas;
            cout << "itt: " << _selectedInt << endl;
